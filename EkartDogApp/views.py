...
def viewcart(request):
    ...
    np=len(c)
    for x in c:
        s=s+x.pid.pcost*x.qty #This line will update the quantity with cost 
    context={}
    context['n']=np 
    context['Products']=c
    context['total']=s  
...
def makepayment(request):
    ...
    for x in orders:
        s=s+x.pid.pcost*x.qty
        oid=x.order_id
    client = razorpay.Client(auth=("rzp_test_eKxAtDO3pIZyWp", "Mm8jZAZzINyTIXD336s8V6ln"))
...
def sendusermail(request):
        ...
        msg,
        "notlagovindaraju2001@gmail.com",
        [uemail],
        fail_silently=False,
        )
